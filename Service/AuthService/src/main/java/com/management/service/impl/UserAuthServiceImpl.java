...
public class UserAuthServiceImpl implements UserAuthService {
     ...
     */
    @Override
    public ResponseResult sendCode(String mail) {
        // Todo: 使用消息队列重构
        // 构建redis键
        ...
        // 在redis中存放五分钟
        template.opsForValue().set(redisKey, text, 30000, TimeUnit.SECONDS);
        // 发送value给邮箱
...
//                "现已通知相关辅导员，请手写5000字书面检讨，并在2022年4月1日17点前交到辅导员办公室。");
        ...
        //OK，万事俱备只欠发送
        sender.send(message);
        return new ResponseResult(2000, "发送完成");
    }
    ...
    @Override
    @Transactional
    public ResponseResult register(UserCreateRequest userCreateRequest) {
        // 查询用户是否已存在
        ...
        // 构建redis键
        String redisKey = "register:" + userCreateRequest.getMail();
        // 查看redis中是否已经存了验证码
        String verifyCode = template.opsForValue().get(redisKey);
        if (StringUtils.hasText(verifyCode)) {
            boolean ok = verifyCode.equals(userCreateRequest.getCode());
            // 验证码错误
            if (!ok) {
